function CB_Map() {
  ...
  cb_map.tile_servers = {
    ...
    5: "https://maptiles.flotte-berlin.org/{z}/{x}/{y}.png",
  };
  ...
  (cb_map.init_map = function () {
    ...
    var map_center = {
      lat: this.settings.map_center_lat,
      lon: this.settings.map_center_lon,
    };
    var delta_lat = this.settings.map_span_lat;
    var delta_long = this.settings.map_span_lon;
    var south_west = L.latLng(
        map_center.lat - delta_lat / 2,
        map_center.lon - delta_long / 2
      ),
      north_east = L.latLng(
        map_center.lat + delta_lat / 2,
        map_center.lon + delta_long / 2
    ...
    var map_options = {
      ...
      maxBounds: bounds,
      maxBoundsViscosity: this.settings.map_bounds_viscosity,
    };
    ...
    var osm = new L.TileLayer(tile_server_url, layer_options);
    ...
    map.on("popupopen", function (e) {
      ...
      jQuery(e.popup._container)
        .find(".cb-map-popup-item-availability")
        .overscroll({
  ...
  cb_map.render_locations = function (data, init, center_position) {
    ...
    jQuery.each(data, function (index, location) {
      ...
      location.items.forEach(function (item) {
        ...
        var item_thumb_image = item.thumbnail
        ...
        popup_items +=
          '<div class="cb-map-popup-item-desc">' + item.short_desc + "</div>";
        popup_items += "</div></div>";
        ...
        if (cb_map.settings.custom_category_colors_marker_icon) {
          item.terms.forEach((term, i) => {
            if (that.color_filter_ids.includes(term)) {
              if (
                !color_filter_cat_id ||
                that.color_filter_ids.indexOf(term) <
                  that.color_filter_ids.indexOf(color_filter_cat_id)
              ) {
      ...
      popup_content +=
        '<span id="location-zoom-in-' +
        that.settings.cb_map_id +
        "-" +
        index +
      ...
      if (that.settings.show_location_opening_hours && location.opening_hours) {
        popup_content +=
          '<div class="cb-map-location-info-opening-hours"><b><i>' +
          cb_map.translation["OPENING_HOURS"] +
          ":</i></b><br>" +
      ...
      if (that.settings.show_location_contact && location.contact) {
        popup_content +=
          '<div class="cb-map-location-info-contact"><b><i>' +
          cb_map.translation["CONTACT"] +
          ":</i></b><br>" +
          location.contact +
          "</div>";
      }
      ...
      if (color_filter_cat_id) {
        marker.on("add", function (e) {
          e.target.valueOf()._icon.style.filter =
            "url(#marker-filter-" + color_filter_cat_id + ")";
        });
      }

      //set map view to location and zoom in
      jQuery(
        "#location-zoom-in-" + that.settings.cb_map_id + "-" + index,
        popup
      ).on("click", function () {
        that.map.closePopup();
        that.map.setView(
    ...
    if (
      ...
      if (Object.keys(data).length > 0) {
        ...
        if (center_position) {
          ...
          var bounds = [
            [
              center_position.lat + max_delta_lat,
              center_position.lon + max_delta_lng,
            ],
            [
              center_position.lat - max_delta_lat,
              center_position.lon - max_delta_lng,
  ...
  cb_map.create_color_filters = function () {
    ...
    jQuery.each(
      cb_map.settings.filter_cb_item_categories,
      function (group_index, group) {
  ...
  cb_map.create_color_filter = function (index, rgb) {
    ...
    var filter_html =
      ...
      index +
      '" color-interpolation-filters="sRGB" x="0" y="0" height="100%" width="100%"><feColorMatrix type="matrix" values="' +
      colorMatrixValues +
      '" /></filter>';
    jQuery("body").append(
